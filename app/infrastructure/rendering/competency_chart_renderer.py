...
def render_radar_chart(values: list[tuple[str, float]], *, title: str = "") -> Figure:
    """Render a Spinnennetz/radar chart of already-normalized percentages (0-100) per label.
    ...
    `category_competency_percentages`. Uses the object-oriented `Figure`
    API directly (not `pyplot`) so repeated calls in this long-running GUI
    process never leak into pyplot's global figure registry.
    """
    ...
    if count == 0:
        fig.add_subplot(111).text(0.5, 0.5, "Keine Daten", ha="center", va="center")
        return fig
...
def render_bar_chart(values: list[tuple[str, float]], *, title: str = "") -> Figure:
    """Render a bar chart of already-normalized percentages (0-100) per label. See `render_radar_chart`."""
    labels = [label for label, _percent in values]
    percents = [percent for _label, percent in values]
