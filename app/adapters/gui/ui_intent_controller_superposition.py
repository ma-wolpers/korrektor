...
class UiIntentControllerSuperpositionMixin:
    ...
    @staticmethod
    def _task_max_points_sum(exam: ExamProject, region_id: str, task_codes: list[str]) -> float:
        region = next((item for item in exam.regions if item.region_id == region_id), None)
        if region is None:
            return 0.0
        max_points_by_code = {task.code: task.max_points for task in region.tasks}
        return sum(max_points_by_code.get(code, 0.0) for code in task_codes)
    ...
    def apply_grade_superposition_immediate(
        ...
        *,
        ...
        """Place each target student's grade (via `exam.grading_scale_snapshot`) at one shared position.

        Requires an assigned Notenschluessel-Snapshot; without one the
        action is rejected outright (no partial/best-effort grading).
        Same all-or-nothing missing-points validation as
        `apply_scored_superposition_immediate`, plus per-student grade
        resolution via `resolve_grade` - a `GradingError` for any single
        student (e.g. below the lowest threshold) aborts the whole action
        with a diagnostic naming that student, same as a missing points
        value.
        """
        ...
        max_points = self._task_max_points_sum(exam, region_id, task_codes)
        area_code = self._area_code_for_region(exam, region_id)
        content_by_student_id: dict[str, str] = {}
        for student in target_students:
            try:
                result = resolve_grade(values_by_student_id[student.student_id], max_points, exam.grading_scale_snapshot)
            except GradingError as exc:
                messagebox.showerror(
                    "Superposition nicht moeglich",
                    f"Fuer {student.display_name} im Bereich {area_code} konnte keine Note ermittelt werden: {exc}",
                )
                return None
