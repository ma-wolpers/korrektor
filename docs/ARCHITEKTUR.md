...
**Bewusste Ausnahme:** `main_window_region_editor.py` liegt mit ~322 Zeilen Code minimal ueber der 300-Zeilen-Konvention - bewusst nicht weiter zerlegt. Der Ueberhang stammt aus einer einzigen, echten Kopplung: Reading-Modus und Extraseiten-Modus teilen sich einen Treeview (`_regions_tree`/`_region_tree_rows`), eine Selektion (`_selected_region_id`/`_selected_region_kind`) und ein Draft-Dict (`_draft_regions`), sodass Treeview-Aufbau, Selektions-Handling, Speichern/Loeschen und die drei geteilten Render-Helfer (`_build_superposed_pixmap`, `_render_pdf_page`, `_wheel_direction`) zusammen bleiben muessen - ein modusbasierter Split wuerde diese Kopplung nur ueber mehrere Dateien verteilen statt sie zu kapseln (siehe die reinen Formatierungs-/Lookup-Helfer, die bereits als `main_window_region_specs.py` ausgelagert sind, weil sie diese Kopplung nicht teilen).
