...
## Unreleased
- Die Auswertung zeigt jetzt zusätzlich ein Kompetenzgrad-Diagramm pro Schüler:in (Spinnennetz oder Balken, wahlweise pro Aufgabe oder pro Kategorie) - aktualisiert sich automatisch beim Personenwechsel oder Umschalten der Ansicht.
- Neuer Button "Auswertung" in der Klausur-Detailansicht: zeigt pro Schüler:in (mit Personen-Navigation) das Gesamtergebnis, die erreichte Note (falls ein Notenschlüssel zugeordnet ist), sowie eine Aufschlüsselung pro Aufgabe und pro Kategorie. Noch nicht bewertete Aufgaben werden als "-" angezeigt statt als 0 Punkte gezählt; ist noch nicht alles bewertet, wird auch keine (ggf. irreführende) Gesamtsumme oder Note angezeigt.
- Neuer Button "Kategorien" in der Klausur-Detailansicht: eine Seite, auf der sich Aufgaben per Drag-and-Drop auf selbst erstellte und benannte Kategorien ziehen lassen (oder zurück auf "Unkategorisiert"). Kategorien lassen sich anlegen, umbenennen, per ▲/▼ umsortieren und löschen (zugeordnete Aufgaben werden dabei nur unkategorisiert, nicht gelöscht). Jede Änderung ist einzeln rückgängig machbar.
- Neuer Bereich "Superposition" im Korrekturmodus (neben dem Supersymbol-Filter): "Punkte einfügen" trägt an einer für alle Personen des aktuellen Bereichs gemeinsamen Position jeweils die individuell erreichte Punktzahl ein, "Note einfügen" entsprechend die individuelle Note (setzt einen zugeordneten Notenschlüssel voraus). Beide sind je eine einzige, rückgängig machbare Aktion für alle betroffenen Personen zusammen. Fehlt für eine Person im gewählten Aufgaben-Umfang noch eine Bewertung, wird die gesamte Aktion mit einer konkreten Fehlermeldung (Person, Aufgabe, Bereich) abgelehnt statt sie nur für einen Teil der Personen auszuführen.
- Neuer Button "Notenschlüssel" in der Klausur-Detailansicht öffnet eine Verwaltung für Notenschlüssel: anlegen (Name, Fach, Jahrgang, Schule, Skala Punktnoten 1-15 oder Schulnoten sehr gut(1)-ungenügend(6), Notenstufen als Prozent:Note-Liste), bearbeiten, sowie archivieren statt löschen sobald ein Notenschlüssel bereits einer Klausur zugeordnet wurde (noch nie verwendete lassen sich weiterhin ganz löschen); zeigt pro Notenschlüssel, in welchen Klausuren er bereits verwendet wurde. In der Detailansicht kann direkt darunter ein aktiver Notenschlüssel per Dropdown der aktuellen Klausur zugeordnet werden - eine spätere Bearbeitung des globalen Notenschlüssels ändert eine bereits zugeordnete Klausur nie rückwirkend, da die zum Zeitpunkt der Zuordnung gültigen Werte fest übernommen werden. Die Zuordnung ist eine einzige, rückgängig machbare Aktion.
...
- Jede einzelne Markierungsaktion im Korrekturmodus (Symbol setzen, verschieben, Groesse/Rotation/Farbe aendern, loeschen, einfuegen, Durchdruecken ein-/ausschalten) wird jetzt sofort gespeichert und ist einzeln per Strg+Z rueckgaengig machbar - vorher blieben diese Aenderungen nur im Speicher und gingen erst beim naechsten "Alle PDFs ueberschreiben" in die Klausurdatei, ohne Undo/Redo-Unterstuetzung. Ein zusammenhaengendes Ziehen einer Markierung zaehlt dabei als eine Aktion (nicht eine pro Mausbewegung); aendert eine Aktion eine ganze "Durchgedrueckte" Gruppe (z. B. Farbe), ist das ebenfalls eine einzige rueckgaengig machbare Aktion fuer die ganze Gruppe. Schlaegt das Speichern fehl, wird die Aenderung nicht angezeigt, sondern zurueckgenommen.
- Strg+Z/Strg+Y springen bei einer Inhaltsaenderung (z. B. Supersymbol, Punkte speichern, Bereich bearbeiten, Umbenennen, Fertig-Checkbox) nicht mehr automatisch zur Klausur-Detailuebersicht zurueck - der aktuelle Modus (z. B. Korrekturmodus), die ausgewaehlte Person und der Bereich bleiben erhalten, nur die betroffene Anzeige wird aktualisiert. Nur beim Anlegen/Loeschen einer Klausur oder beim Wechsel des JSON-Ablageordners bleibt der Ruecksprung zur Uebersicht bestehen, da sich dort die Klausur-Identitaet selbst aendert.
- Neuer Superbutton im Korrekturmodus: Neben der bisherigen "Fertig korrigiert"-Checkbox pro Person gibt es jetzt darunter eine zweite Checkbox "Bereich: alle als fertig markieren" fuer den aktuell gewaehlten Bereich. Sind noch keine Person fertig (0/N) und haben alle Personen alle Aufgaben des Bereichs bewertet, setzt ein Klick alle auf fertig. Ist mindestens eine Person bereits fertig (egal ob gemischt oder alle) zeigt die Checkbox das als Zwischenzustand ("N von M Personen fertig") und ein Klick setzt alle wieder auf offen - unabhaengig davon, ob inzwischen noch alle Aufgaben bewertet sind. Die einzelnen Fertig-Checkboxen bleiben unveraendert nutzbar und koennen den Sammelstatus jederzeit einzeln ueberschreiben; ein Bereichs-Klick ist eine einzige, rueckgaengig machbare Aktion.
- Neues Supersymbol im Korrekturmodus: Im Bereich "Supersymbol (Filter)" waehlt man eine Aufgabe (oder "Summe aller Aufgaben"), eine Bedingung (`<`, `≤`, `=`, `≥`, `>`) und einen Punktwert, dann "Vorschau anzeigen". Alle Schueler:innen, die die Bedingung erfuellen, werden ueberlagert als eine Vorschauseite angezeigt; ein Klick darauf setzt das aktuell gewaehlte Markierungssymbol bei allen Treffern gleichzeitig. Das ist eine einzige, rueckgaengig machbare Aktion. Erfuellt niemand die Bedingung, wird das mit einer Fehlermeldung abgelehnt statt eine leere Vorschau anzuzeigen; spaetere Punkte-Aenderungen bewegen bereits gesetzte Supersymbole nicht automatisch.
- Bug behoben (bereits vorher vorhanden, nicht neu): In der Uebersicht zeigten die Spalten `Korrektur %` und `Korrigiert` immer 0, selbst wenn ueberall "Fertig korrigiert" markiert wurde. Ursache: Diese Spalten lasen ein `is_corrected`-Feld an den Bereichen, das im gesamten Programm nirgends gesetzt wird. Sie nutzen jetzt denselben Fortschritt, der auch in der Klausur-Detailansicht als "Fertig korrigiert X/Y" angezeigt wird. Ausserdem: die Titelzeile hiess versehentlich "Uebersicht" statt "Übersicht".
- Neuer Namenmodus: Ueber den Button `Namen` in der Klausur-Detailansicht laesst sich ein gemeinsamer Namensbereich fuer alle Schueler:innen-PDFs ziehen (mit Superseite zur Ausrichtung, Seitennavigation wie im Einlesemodus). Ein Klick auf `Namen erfassen` fuehrt zur Namenserfassung: per Pfeiltasten durch jede PDF gehen, den erkannten Namen im automatisch fokussierten Feld eintragen (Speichern beim Verlassen des Feldes) und am Ende alle PDFs in einem Schritt umbenennen. Umbenennen ist eine einzige, rueckgaengig machbare Aktion; Extraseiten-Zuordnungen und Markierungen werden automatisch auf die neuen Dateinamen umgestellt, `student_id` und damit die Punktezuordnung in `korrektor_scores.csv` bleiben unveraendert. Vor dem Umbenennen wird geprueft, dass alle PDFs im Klausurordner eindeutig den Schueler:innen zugeordnet sind und keine Zielnamen mit fremden Dateien kollidieren; im Fehlerfall wird nichts veraendert. (Nachgebessert: der `Namen erfassen`-Button war unsichtbar, da er beim Moduswechsel hinter das bereits gepackte Canvas/Editor-Splitpane sortiert wurde statt davor; ausserdem blieb der themenfremde `Einlesen abschliessen`-Button im Namenmodus sichtbar.)
- Korrekturmodus-Bug behoben: Nach dem Loeschen eines Bereichs im Einlesemodus konnten neu gezogene Bereiche denselben Buchstaben wie ein bereits bestehender bekommen; im Korrekturmodus war dann nur noch einer der beiden Bereiche auswaehlbar, obwohl beide gespeichert waren. Bereichs-Buchstaben werden jetzt kollisionssicher vergeben, und eine Klausur-Datei mit widerspruechlichen Bereichsdaten wird beim Laden klar als fehlerhaft gemeldet statt Bereiche stillschweigend zu verlieren.
